...
const SEND_URL = process.env.LTFS_OTP_SEND_URL || 'https://apiclouduat.ltfs.com:1132/LTFSME/api/sendBumraPosterOtp/';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const payloadOverride: any = body?.payload; // optional exact plaintext object to encrypt

    if (!FLS_ID || !LEND_TOKEN) {
      return NextResponse.json({ success: false, error: 'Server OTP credentials not configured' }, { status: 500 });
    }

    if (!phone && !payloadOverride) {
      return NextResponse.json({ success: false, error: 'phone or payload is required' }, { status: 400 });
    }

    // Build plaintext payload (override wins)
    const plaintextObj = payloadOverride ?? { mobileNumber: phone };
    ...
    // Attempt decrypt if a `body` field is present
    ...
    return NextResponse.json({ success: false, error: e?.message || 'OTP send failed' }, { status: 500 });
  }
}
