...
create table if not exists public.bowling_attempts (
  ...
  predicted_kmh numeric(6,2) not null check (predicted_kmh >= 0 and predicted_kmh <= 250),
  similarity_percent numeric(5,2) not null check (similarity_percent >= 0 and similarity_percent <= 100),
  intensity_percent numeric(5,2) not null check (intensity_percent >= 0 and intensity_percent <= 100),
  ...
  location text null,
  device text null,
  duration_seconds real null check (duration_seconds is null or duration_seconds >= 0),
...
order by predicted_kmh desc, similarity_percent desc, created_at asc;
...
with ranked as (
  select
    ...
    row_number() over (
      ...
      order by ba.predicted_kmh desc, ba.similarity_percent desc, ba.created_at asc
    ) as rn
