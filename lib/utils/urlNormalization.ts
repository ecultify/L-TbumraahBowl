...
export function isValidVideoUrl(url: string | null | undefined): boolean {
  ...
  try {
    ...
    console.error('❌ [URL Normalization] Error validating URL:', error);
    return false;
  }
}
 ...
 */
export function extractVideoFilename(url: string | null | undefined): string | null {
  if (!url) return null;
  ...
  try {
    ...
    console.error('❌ [URL Normalization] Error extracting filename:', error);
    return null;
  }
}
